...
void ApplicationFunctionSet::Drive(OwlCarMotionControl direction, uint8_t speed) {
    ApplicationFunctionSet_OwlCarMotionControl(direction, speed);
}
...
void ApplicationFunctionSet::ApplicationFunctionSet_Exploration(void)
{
  ...
  {
    float getAnaloguexxx_R = AppITR20001.DeviceDriverSet_ITR20001_getAnaloguexxx_R(); //获取传感器数据
...
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void)
{
  ...
  if (Application_OwlCarxxx0.Functional_Mode == TraceBased_mode)
  {
    ...
    if (1 == function_xxx(TrackingData_M, TrackingDetection_S, TrackingDetection_E)) //前进 2==010
    {
      ...
      BlindDetection = true;
      /*控制左右电机转动：实现匀速直行*/
      ApplicationFunctionSet_OwlCarMotionControl(Forward, 200);
